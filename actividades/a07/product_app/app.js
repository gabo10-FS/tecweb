...
$(document).ready(function(){
                    ...
                    productos.forEach(producto => {
                        ...
                        template += `
                            <tr productId="${producto.id}">
                                <td>${producto.id}</td>
                                <td>
                                    <a href="#" class="product-item" productName="${producto.nombre}">${producto.nombre}</a>
                                </td>
    ...
    /*$(document).on('click', '.product-item', function(){
        let element = $(this)[0];
        console.log(element);
        let name = $(element).attr('productName');
        console.log(name);
        $.post('backend/product-single.php', {name}, function(response){
            let product = JSON.parse(response);
            $('#name').val(product.nombre);
            $('#productId').val(product.id);
            console.log(product);
